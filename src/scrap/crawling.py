...
def get_info(page_index , url , route , veriable):
    ...
    def get_saramin():
        ...
        new_url = veriable.replace("recruitPage=1", f"recruitPage={page_index}")       #사람인은 recruitPage 를 인덱스로 잡는다
        response = requests.get(new_url)   
        html = response.text
        soup = BeautifulSoup(html, 'html.parser')
        items = soup.select("div.content")
        absolute_path = os.path.abspath(route[0])                                      #절대경로 설정을 위한 os 모듈 사용 (사람인의 경로는 route 리스트 형태로 인덱스 0번에 있습니다.)
